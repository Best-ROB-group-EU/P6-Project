...
#rosrun yolact eval.py --trained_model=src/yolact/src/weights/yolact_base_157_4409_interrupt.pth --score_threshold=0.15 --top_k=15 --image=src/yolact/src/test_image/test_image1.png:src/yolact/src/test_result/result_image.png
...
def fit_square(points):
    multiplier = 1
    thresh = 0.05
    size = cv2.contourArea(points)
    while 1:
        epsilon = multiplier*cv2.arcLength(points,True)
        approx = cv2.approxPolyDP(points,epsilon,True)
        p_size = cv2.contourArea(approx)
        diff = abs(size-p_size)
        if diff < thresh*size:
            break
        else:
...
def fit_shape(img_data):
    sidewalk_id = -1
    largest = 0
    # Find sidewalk id with largest size
    for i in range(len(img_data[0])):
        if img_data[0][i] == 2:
            size = torch.sum(img_data[3][i])
            if size > largest:
                largest = size
                sidewalk_id = i
    # If sidewalk, reduce contour to simplest polygon that contain 98% of the sidewalk
    if sidewalk_id > -1:
        mask = img_data[3][sidewalk_id]
        mask = mask.cpu().detach().numpy()
        mask = np.array(mask * 255, dtype = np.uint8)
        contour, hier = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        size = cv2.contourArea(contour[0])
        # Find work region for depression check since we got the mask here already
        depr_region = find_work_region(mask)
        thresh = 0.02
        multiplier = 1
        while 1:
            epsilon = multiplier*cv2.arcLength(contour[0],True)
            approx = cv2.approxPolyDP(contour[0],epsilon,True)
            poly = cv2.approxPolyDP(approx, 0.0001, True)
            p_size = cv2.contourArea(poly)
            diff = abs(size-p_size)
            if diff < thresh*size:
                break
            else:
                multiplier = multiplier - 0.005
        # Convert to list for publishing
        point_list = approx.tolist()

        for j in range(len(point_list)):
            point_list[j] = point_list[j][0]

        return point_list, depr_region

def get_tile_corners(img_data):
    #Postprocess returns classes, scores, boxes and masks
    #Class 1 is tiles, 2 is sidewalk, 3 is cobblestone
    tile_ids = []
    for i in range(len(img_data[0])):
        if img_data[0][i] == 1:
            tile_ids.append(i)

    #Fit square to each contoured mask
    corners = []
    for i in range(len(tile_ids)):
        mask = img_data[3][tile_ids[i]]
        mask = mask.cpu().detach().numpy()
        mask = np.array(mask * 255, dtype = np.uint8)
        contour, hier = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        l_cnt = 0
        if len(contour) > 1:
            for j in range(len(contour)):
                size = cv2.contourArea(contour[j])
                if size > l_cnt:
                    l_cnt = size
                    idx = j
        else:
            idx = 0
        fit = fit_square(contour[idx])
...
def sidewalk_publisher(sidewalk):
    sidewalk_str = ", ".join(str(x) for x in sidewalk)
    msg = String()
    msg.data = sidewalk_str
    s_pub.publish(msg)


def workregion_publisher(work_region):
    workregion_str = ", ".join(str(x) for x in work_region)
    msg = String()
...
def evaluate(net:Yolact, dataset, frame, train_mode=False):
    ...
    if args.image is not None:
        ...
        elif 'sidewalk' in args.image:
            evalsidewalk(net, frame)
        else:
...
def evalsidewalk(net:Yolact, frame):
    ...
    cv2.imwrite('src/yolact/src/test_result/result_Image.png', img_numpy)
...
def image_callback(data):
    ...
    evaluate(net, dataset, current_frame)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        net.eval() #Puts net in evaluation mode, torch function
        print(' Done.')
